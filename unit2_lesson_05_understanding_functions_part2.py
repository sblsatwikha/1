...
def test_function_call_with_keyword_arguments():
    assert [10,200,3,4] == demo(10,200)
    assert [10,20,3,4] == demo(10, 20)
    assert [10,20,30,4] == demo(10, 20, 30)
    assert [100,20,3,4] == demo(100,second=20)
    assert [1000,20,30,4] == demo(1000,second=20, third=30)
    assert [10,2000,30,4] == demo(first=10,second=2000,third=30)
    assert [10,20000,30,4] == demo(10,20000, third=30)
...
def simple_format(format,*args):
    ...
    while i<(len(format)) :
        if format[i]=="%":
            j=int(format[i+1])
            if j<len(args):
                format=format[0:i]+args[j]+format[i+2:]
        i=i+1
    return format
