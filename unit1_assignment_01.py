...
def get_odds_desc(count):
    ...
    """
    s=list(range(1,2*count,2))
    s.reverse()
    return s
def test_get_odds_desc():
...
def get_multiples_desc(number, count):
    ...
    lists lesson.
    """
    a=list(range(number,(number*count)+1,number))
    a.reverse()
    return a
...
def ascii_distance(first, second):
     ...
     """
     t=abs(ord(first)-ord(second))
     return t # use 2 builtin functions to get your work done.
