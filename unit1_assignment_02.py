...
def get_lower_to_upper_dict():
    ...
    """
    t=dict(map(lambda x:(x,x.upper()),string.ascii_lowercase))
    return t
...
def get_encoding_dict():
    ...
    """
    t=dict((map(lambda x:(x,ord(x.lower())-ord("a")+1),string.ascii_letters)))
    return t
...
def capitalize_names(input):
    ...
    """
    input=list(x.capitalize() for x in input)
    return input
